...
describe('DELETE /api/users/:id TESTS', () => {

	it('should return 404 and error message', async () => {
		const validUUID = "f2d9c476-627f-4fa4-abee-3914adeb778a";
		const res = await request(requestListener).delete(`/api/users/${validUUID}`);
	...
	it('should return 400 and error message', async () => {
		...
		const res = await request(requestListener).delete(`/api/users/${invalidUUID}`);
